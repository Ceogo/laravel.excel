    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($cabinets as $cabinet)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <form action="{{ route('cabinets.destroy', $cabinet->id) }}" method="POST" onsubmit="return confirm('Вы уверены, что хотите удалить кабинет?');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Кабинеты не найдены</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
